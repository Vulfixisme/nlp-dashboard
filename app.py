"""
SOYSIGNAL v2.0 - NLP Sentiment Dashboard
Soybean commodity news sentiment analysis & market price trends
Menggunakan: dataset.csv (800 records, hd1–hd10 per row)
             TF-IDF N-Grams (1,2) + Decision Tree — dilatih ulang tiap sesi
Palet warna: Agro Light — Forest Green, Olive, Sage, Cream
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# COLOR PALETTE : SOYSIGNAL Agro Light
# ═══════════════════════════════════════════════════════════════════════════════
C_BG       = "#FEFEFE"
C_CARD     = "#F0F3E9"
C_BORDER   = "#A8ACA5"
...
[data-testid="stSidebar"] .stRadio label {{
    color: {C_TEXT}; font-size: 0.82rem; padding: 5px 0; transition: 0.2s;
}}
[data-testid="stSidebar"] .stRadio label:hover {{ color: {C_GREEN}; }}

.metric-card {{
    background: {C_CARD}; border: 1px solid {C_BORDER}; border-radius: 12px;
    padding: 18px; text-align: center; box-shadow: 0 1px 6px rgba(75,103,54,0.07);
}}
.metric-label {{
    font-size: 0.68rem; color: {C_MUTED}; letter-spacing: 0.12em;
...
.section-header {{
    font-size: 0.72rem; letter-spacing: 0.18em; color: {C_GREEN};
    text-transform: uppercase; border-bottom: 1.5px solid {C_BORDER};
    padding-bottom: 8px; margin: 26px 0 14px 0; font-weight: 700;
}}

.pipe-row {{ display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin: 14px 0; }}
.pipe-node {{
    border: 1px solid {C_BORDER}; color: {C_MUTED}; border-radius: 8px;
    padding: 7px 14px; font-size: 0.78rem; background: {C_BG};
}}
.pipe-node.active {{
    background: {C_SAGE}; font-weight: 700; color: {C_GREEN}; border-color: {C_GREEN};
}}
.pipe-arrow {{ color: {C_ACCENT}; font-size: 1rem; }}

.stButton > button {{
    width: 100%; background-color: {C_GREEN}; color: {C_BG};
    border: 1px solid {C_GREEN}; border-radius: 10px; padding: 0.55rem 1rem;
    transition: all 0.3s ease; font-weight: 600;
}}
.stButton > button:hover {{
    background-color: #3A5228; color: {C_BG}; border: 1px solid #3A5228;
...
textarea:focus {{
    border: 1px solid {C_GREEN} !important; box-shadow: 0 0 0 1px {C_GREEN} !important;
}}

[data-testid="stDataFrame"] {{
    background-color: {C_BG}; border: 1px solid {C_BORDER};
...
.pill {{ display: inline-block; padding: 4px 12px; border-radius: 14px; font-size: 0.72rem; font-weight: 600; }}
.pill-red  {{ background: #FDECEA; color: {C_RED};    border: 1px solid {C_RED}; }}
.pill-green {{ background: {C_SAGE}; color: {C_GREEN}; border: 1px solid {C_GREEN}; }}
.pill-yellow {{ background: #FEF3E2; color: {C_YELLOW}; border: 1px solid {C_YELLOW}; }}

::-webkit-scrollbar {{ width: 8px; }}
::-webkit-scrollbar-track {{ background: {C_CARD}; }}
::-webkit-scrollbar-thumb {{ background: {C_BORDER}; border-radius: 8px; }}
::-webkit-scrollbar-thumb:hover {{ background: {C_GREEN}; }}

.custom-hr {{ border: none; border-top: 1.5px solid {C_BORDER}; margin: 20px 0; }}
</style>
...
def _clean(text: str) -> str:
    """Lowercase → hapus non-alpha → collapse whitespace."""
    text = str(text).lower()
...
def load_data():
    ...
    # ── harga bulanan ─────────────────────────────────────────────────────────
    df["monthly_average"] = (
        df["average"].astype(str).str.replace(",", ".", regex=False).astype(float)
    )

    # ── tanggal (format YYYY-MM → YYYY-MM-01) ────────────────────────────────
    df["date"] = pd.to_datetime(
        df["Date"].astype(str).str.strip().apply(
            lambda x: x + "-01" if len(x) == 7 else x
        )
    )
    df["year"]  = df["date"].dt.year
    df["month"] = df["date"].dt.month

    # ── raw_text & clean_text dari hd1–hd10 ──────────────────────────────────
    hd_cols = [f"hd{i}" for i in range(1, 11)]
    df["headline_count"] = df[hd_cols].notna().sum(axis=1)
    df["raw_text"] = df[hd_cols].fillna("").apply(
        lambda r: " ".join([x for x in r if str(x).strip()]), axis=1
    )
    df["clean_text"] = df["raw_text"].apply(_clean)
    df["len_raw"]   = df["raw_text"].apply(lambda x: len(x.split()))
    df["len_clean"] = df["clean_text"].apply(lambda x: len(x.split()))
    df["headline_summary"] = df["raw_text"].str[:65] + "…"
    return df


# ═══════════════════════════════════════════════════════════════════════════════
# MODEL TRAINING  (TF-IDF N-Grams (1,2) + Decision Tree)
# ═══════════════════════════════════════════════════════════════════════════════
@st.cache_resource
def train_model(_df):
    """
    Latih TF-IDF (unigram+bigram) + DecisionTree langsung dari dataset.csv.
    Di-cache agar tidak re-train setiap interaksi UI.
    """
    X, y = _df["clean_text"], _df["Label3"]
    ...
    dt = DecisionTreeClassifier(
        max_depth=8, min_samples_split=4, random_state=42
    )
...
with st.sidebar:
    ...
    <div style='margin-bottom:6px;'>
      <span style='color:{C_GREEN}; font-size:1.3rem; font-weight:700;'>SOYSIGNAL</span><br/>
      <span style='color:{C_MUTED}; font-size:0.7rem;'>v2.0 · NLP Sentiment</span>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
...
# HELPER WIDGETS
# ═══════════════════════════════════════════════════════════════════════════════
def metric_card(label, value, sub, color=C_GREEN):
    return f"""
...
# ═══════════════════════════════════════════════════════════════════════════════
if page == "Overview":
    st.markdown(f"<h2 style='color:{C_TEXT};'>Market Sentiment Dashboard</h2>", unsafe_allow_html=True)
    st.markdown(
    ...
    st.markdown("""
    <div class="pipe-row">
      <span class="pipe-node">Raw Headlines (hd1–hd10)</span>
      <span class="pipe-arrow">→</span>
      <span class="pipe-node">Cleaning</span>
      <span class="pipe-arrow">→</span>
      <span class="pipe-node">TF-IDF N-Grams (1,2)</span>
      <span class="pipe-arrow">→</span>
    ...
    for col, lbl, name, bg_pill in [
        (cd1, -1, "Down (-1)",    "#FDECEA"),
        (cd2,  0, "Neutral (0)", "#FEF3E2"),
        (cd3,  1, "Up (+1)",     C_SAGE),
    ]:
        count = vc.get(lbl, 0)
        pct   = count / total * 100
    ...
    ax.scatter(df["date"], df["monthly_average"], c=colors_line, s=30, zorder=2,
               edgecolors=C_BG, linewidths=0.5)
    ...
    ax.legend(handles=patches, fontsize=7, loc="upper right",
              framealpha=0.8, facecolor=C_CARD, edgecolor=C_BORDER)
    fig.tight_layout()
    st.pyplot(fig)
    plt.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='color:{C_TEXT};'>Exploratory Data Analysis</h2>", unsafe_allow_html=True)
    st.markdown(
        f"<span style='color:{C_MUTED}; font-size:0.82rem;'>"
        "Eksplorasi distribusi, tren, dan korelasi dalam dataset</span>",
        unsafe_allow_html=True,
    )
    ...
    with r1c1:
        ...
        ax.axvline(df["monthly_average"].mean(),   color=C_GREEN,  lw=1.5, linestyle="--",
                   label=f"Mean: {df['monthly_average'].mean():.2f}")
        ax.axvline(df["monthly_average"].median(), color=C_YELLOW, lw=1.5, linestyle=":",
        ...
        ax.legend(fontsize=7, framealpha=0.8, facecolor=C_CARD, edgecolor=C_BORDER)
    ...
    with r2c1:
        ...
        fig, ax = plt.subplots(figsize=(6, 3.5))
        ax.plot(yearly.index, yearly.values, color=C_GREEN, marker="o", markersize=5, lw=2)
        ax.fill_between(yearly.index, yearly.values, alpha=0.10, color=C_ACCENT)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='color:{C_TEXT};'>Text Preprocessing</h2>", unsafe_allow_html=True)
    st.markdown(
        f"<span style='color:{C_MUTED}; font-size:0.82rem;'>"
        "Transformasi teks mentah (hd1–hd10) → teks bersih siap diekstrak fiturnya</span>",
        unsafe_allow_html=True,
    )

    section("TAHAPAN PREPROCESSING")
    steps = [
        ("1. Gabungkan Headlines",   "Menggabungkan hd1–hd10 menjadi satu string raw_text per baris"),
        ("2. Lowercase",             "Mengubah semua huruf menjadi lowercase"),
        ("3. Remove Punctuation",    "Menghapus tanda baca dan karakter khusus"),
        ("4. Remove Numbers",        "Menghapus angka yang tidak informatif"),
        ("5. Collapse Whitespace",   "Menghapus spasi ganda / leading-trailing"),
        ("6. Cleaned Text",          "Teks siap untuk ekstraksi fitur TF-IDF N-Grams (1,2)"),
    ]
    cols = st.columns(3)
    for i, (name, desc) in enumerate(steps):
        color = C_GREEN if i == 5 else C_ACCENT
        with cols[i % 3]:
            st.markdown(
    ...
    with cc2:
        ...
        st.markdown(
            f"""<div style='background:{C_SAGE}; border:1px solid {C_BORDER}; border-radius:8px;
    ...
    wc1.markdown(metric_card("Avg Raw Words",   f"{df['len_raw'].mean():.0f}",   "kata per record", C_TEXT),    unsafe_allow_html=True)
    wc2.markdown(metric_card("Avg Clean Words", f"{df['len_clean'].mean():.0f}", "kata per record", C_GREEN),   unsafe_allow_html=True)
    ...
    ax1.hist(df["len_raw"],   bins=15, color=C_BORDER, alpha=0.8, edgecolor=C_BG, label="Raw")
    ...
    ax1.legend(fontsize=7, framealpha=0.8, facecolor=C_CARD, edgecolor=C_BORDER)

    ax2.scatter(df["len_raw"], df["len_clean"],
                c=[LABEL_COLORS[l] for l in df["Label3"]], s=25, alpha=0.65,
                edgecolors=C_BG, linewidths=0.4)
    ...
    patches = [mpatches.Patch(color=LABEL_COLORS[k], label=LABEL_NAMES[k]) for k in [-1, 0, 1]]
    ax2.legend(handles=patches, fontsize=7, framealpha=0.8, facecolor=C_CARD, edgecolor=C_BORDER)
    fig.tight_layout(); st.pyplot(fig); plt.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='color:{C_TEXT};'>Feature Extraction</h2>", unsafe_allow_html=True)
    st.markdown(
        f"<span style='color:{C_MUTED}; font-size:0.82rem;'>"
        "TF-IDF dengan N-Grams (unigram + bigram) — dilatih langsung dari dataset</span>",
        unsafe_allow_html=True,
    )
    ...
    fi1.markdown(metric_card("N-Gram Range",  "(1, 2)",          "unigram + bigram", C_TEXT),   unsafe_allow_html=True)
    fi2.markdown(metric_card("Max Features",  "500",             "fitur teratas",    C_MUTED),  unsafe_allow_html=True)
    fi3.markdown(metric_card("Min DF",        "2",               "min dokumen",      C_YELLOW), unsafe_allow_html=True)
    fi4.markdown(metric_card("Vocab Size",    str(len(vec.vocabulary_)), "unique n-grams", C_GREEN), unsafe_allow_html=True)
    ...
    colors = [C_GREEN if v > np.mean(values) else C_ACCENT for v in values]
    bars = ax.barh(names[::-1], values[::-1], color=colors[::-1], alpha=0.85, height=0.65)
    ax.set_xlabel("Feature Importance", fontsize=8)
    ax.set_title("Feature Importance – Decision Tree + TF-IDF N-Grams (1,2)", fontsize=9, color=C_TEXT)
    ...
    bar_colors = [C_GREEN if i % 2 == 0 else C_ACCENT for i in range(len(words_))]
    ax.bar(words_, counts_, color=bar_colors, alpha=0.8, edgecolor=C_BG)
    ...
    for col, label, lname, color in [
        ...
        with col:
            st.markdown(
                f"<div class='section-header' style='color:{color};'>{lname}</div>",
                unsafe_allow_html=True,
            )
            for w, c in top10:
                pct = c / max(len(words), 1) * 100
                st.markdown(
                    f"""<div style='display:flex; justify-content:space-between; font-size:0.75rem;
                    border-bottom:1px solid {C_SAGE}; padding:5px 0;'>
                    <span style='color:{C_TEXT};'>{w}</span>
                    <span style='color:{color}; font-weight:600;'>{c}
                    <span style="color:{C_MUTED}; font-weight:400;">({pct:.1f}%)</span></span></div>""",
                    unsafe_allow_html=True,
                )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='color:{C_TEXT};'>Model Evaluation</h2>", unsafe_allow_html=True)
    st.markdown(
        f"<span style='color:{C_MUTED}; font-size:0.82rem;'>"
        "Performa Decision Tree (max_depth=8) + TF-IDF N-Grams (1,2) pada test set</span>",
        unsafe_allow_html=True,
    )

    acc = model["accuracy"]
    cr  = model["classification_report"]
    n_test = len(model["y_test"])
    ...
    me4.markdown(metric_card("Test Size", str(n_test),                               "records (20%)",     C_MUTED),  unsafe_allow_html=True)
    ...
    with col_cm:
        ...
        im = ax.imshow(cm, cmap="YlGn", aspect="auto", vmin=0)
        ...
        for i in range(3):
            for j in range(3):
                ax.text(j, i, str(cm[i, j]), ha="center", va="center",
                        ...
                        color=C_BG if cm[i, j] > cm.max()/2 else C_TEXT)
    ...
    colors = [C_GREEN if s >= cv_scores.mean() else C_ACCENT for s in cv_scores]
    ...
    ax.legend(fontsize=8, framealpha=0.8, facecolor=C_CARD, edgecolor=C_BORDER)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='color:{C_TEXT};'>Sentiment Prediction</h2>", unsafe_allow_html=True)
    st.markdown(
        f"<span style='color:{C_MUTED}; font-size:0.82rem;'>"
        "Masukkan judul berita soybean untuk diprediksi sentimennya</span>",
        unsafe_allow_html=True,
    )

    section("INPUT TEKS BERITA")

    # ── Session-state untuk tombol contoh ────────────────────────────────────
    ...
    if sc1.button("Contoh Positif"):
        st.session_state["pred_input"] = (
            "brazil soybean export record high farmer selling strong demand china "
            "positive outlook production increase record harvest"
        )
    if sc2.button("Contoh Negatif"):
        st.session_state["pred_input"] = (
            "dry weather impacts soybean yields concern disease spread harvest delay "
            "production estimate downgraded crop condition poor"
        )
    if sc3.button("Contoh Netral"):
        st.session_state["pred_input"] = (
            "usda report weekly soybean market update crop condition slight change "
            "brazil argentina acreage planted record"
        )

    input_text = st.text_area(
        "Masukkan headline / ringkasan berita:",
        value=st.session_state["pred_input"],
        placeholder="Contoh: Brazil soybean export record high while Argentina farmers hold stocks...",
        height=120,
        label_visibility="collapsed",
    )
    ...
    if st.button("PREDIKSI SENTIMEN", use_container_width=True):
        if not input_text.strip():
            ...
            lbl_map = {
                -1: ("DOWN",    C_RED,    "pill-red"),
                 0: ("NEUTRAL", C_YELLOW, "pill-yellow"),
                 1: ("UP",      C_GREEN,  "pill-green"),
            }
            label_text, color, pill_class = lbl_map[pred]

            st.markdown(
                f"""<div class='pred-box' style='border:2px solid {color};'>
                  <div style='font-size:0.72rem; color:{C_MUTED}; margin-bottom:8px; letter-spacing:.1em;'>PREDIKSI SENTIMEN</div>
                  <div style='font-size:2.8rem; font-weight:700; color:{color}; letter-spacing:.08em;'>{label_text}</div>
            ...
            for cls, prob in zip(classes, proba):
                ...
                st.markdown(
                    ...
                    <div style='width:90px; font-size:0.75rem; color:{clr}; font-weight:600;'>{LABEL_NAMES[cls]}</div>
                    <div style='flex:1; background:{C_SAGE}; border-radius:4px; height:18px; overflow:hidden;'>
                      <div style='width:{prob*100:.1f}%; height:100%; background:{clr}; border-radius:4px;'></div>
                    </div>
                    <div style='width:50px; font-size:0.78rem; color:{C_TEXT}; text-align:right; font-weight:600;'>{prob*100:.1f}%</div>
                    </div>""",
                    unsafe_allow_html=True,
                )
    ...
    sample_n  = st.slider("Tampilkan n record dari dataset:", 5, 50, 10)
    ...
    st.markdown(
        f"<div style='font-size:0.82rem; color:{C_GREEN}; margin-top:8px;'>"
        f"Batch Accuracy: <strong>{batch_acc:.1f}%</strong> dari {sample_n} record</div>",
        unsafe_allow_html=True,
    )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='color:{C_TEXT};'>Conclusion</h2>", unsafe_allow_html=True)
    ...
    with cc1:
        st.markdown(
            ...
            <div style='color:{C_GREEN}; font-weight:700; font-size:0.85rem; margin-bottom:10px;'>Tujuan Penelitian</div>
            <div style='color:{C_TEXT}; font-size:0.8rem; line-height:1.8;'>
            Membangun sistem klasifikasi sentimen berita komoditas soybean untuk memprediksi arah pergerakan
            harga pasar bulanan menggunakan pendekatan NLP berbasis TF-IDF N-Grams (1,2) dan Decision Tree
            yang dilatih langsung dari dataset.csv.
            </div></div>""",
            unsafe_allow_html=True,
        )
    with cc2:
        n_down = df[df["Label3"] == -1].shape[0]
        st.markdown(
            f"""<div class='metric-card' style='text-align:left;'>
            <div style='color:{C_ACCENT}; font-weight:700; font-size:0.85rem; margin-bottom:10px;'>Hasil Utama</div>
            <div style='color:{C_TEXT}; font-size:0.8rem; line-height:1.8;'>
            Model terbaik: Decision Tree + CountVec Bigrams (2,2) dengan akurasi
            <span style='color:{C_GREEN}; font-weight:700;'>{model['accuracy']*100:.2f}%</span> pada test set.
            Sentimen <em>Neutral</em> paling banyak ({df[df['Label3']==0].shape[0]} record).
            Kata kunci dominan bigram: <em>soybean crop, planted record, condition unchanged</em>.
            </div></div>""",
            unsafe_allow_html=True,
        )
    ...
    section("Notes")
    findings = [
        ("Dataset",                f"{len(df)} record (800 baris, ~10 headline per bulan). Setiap baris mewakili satu headline dalam satu periode.", C_MUTED),
        ("Kualitas Headline",      "Headline hd1–hd10 digabung sebagai raw_text. Representasi ini memungkinkan bigram lintas headline ter-capture.", C_MUTED),
        ("Dominasi Tema",          "Bigram 'soybean crop', 'planted record', 'condition unchanged' mendominasi — domain-spesifik dengan noise rendah.", C_MUTED),
        ("Ketidakseimbangan Kelas",f"Neutral: {df[df['Label3']==0].shape[0]}, Down: {df[df['Label3']==-1].shape[0]}, Up: {df[df['Label3']==1].shape[0]} — class_weight='balanced' digunakan untuk mengatasi ini.", C_YELLOW),
        ("TF-IDF N-Grams (1,2)",   "Gabungan unigram + bigram terbukti lebih informatif — menangkap kata tunggal ('soybean') sekaligus frasa kontekstual ('soybean harvest', 'planted record').", C_GREEN),
        ...
        for col, item in [(fc1, findings[i]), (fc2, findings[i+1] if i+1 < len(findings) else None)]:
            if item:
                ...
                col.markdown(
                    ...
                    <div style='color:{clr}; font-size:0.82rem; font-weight:700; margin-bottom:6px;'>{title}</div>
    ...
    sc1.markdown(metric_card("Total Data",    f"{len(df)} records", "Jan 2014 – Des 2020",  C_TEXT),   unsafe_allow_html=True)
    sc2.markdown(metric_card("Model",                 "DT - N-Grams ",         C_ACCENT), unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        "SOYSIGNAL v2.0 · NLP Sentiment Analysis · Soybean Commodity Market · 2014–2020"
        "</div>",
        unsafe_allow_html=True,
    )